...
import { getCountries, getCountryCallingCode, CountryCode, getExampleNumber, isValidPhoneNumber } from 'libphonenumber-js';
...
const GLOBAL_COUNTRIES = [...priorityCountries, ...remainingCountries];
...
export default function Login() {
  ...
  const selectedCountry = GLOBAL_COUNTRIES[countryIndex]?.code;
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        const valid = isValidPhoneNumber(fullWhatsAppNumber, country.code);
        if (!valid) {
          const expectedDigits = getExpectedDigits(country.code);
          throw new Error(
            expectedDigits
              ? `Local WhatsApp number must be exactly ${expectedDigits} digits long (excluding any leading zero).`
              : 'Invalid WhatsApp number for the selected country.'
          );
        }
              ...
              <div className="space-y-2">
                ...
                <div className="relative group flex items-center bg-background/40 border border-border-main rounded-2xl relative z-10 h-14">
                  ...
                  <input
                    ...
                    value={whatsappLocal}
                    onChange={(e) => setWhatsappLocal(e.target.value.replace(/[^0-9]/g, '').slice(0, 15))}
                  />
                </div>
                <p className="hint-text text-[9px] text-text-muted italic ml-1 mt-1">
                ...
                try {
                  if (isSignUp) {
                    ...
                    if (!valid) {
                      ...
                      throw new Error(
                        expectedDigits
                          ? `Local WhatsApp number must be exactly ${expectedDigits} digits.`
                          : 'Invalid WhatsApp number for the selected country.'
                      );
                    }
